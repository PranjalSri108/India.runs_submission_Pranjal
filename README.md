...
│   ├── build_results.py     # writes results.json (drift-proof demo header numbers)
│   └── results.json         # current metrics for the demo header (regenerated)
├── scripts/
...
The header metric strip (ranking quality + constraints met) is read from
`eval/results.json`, regenerated by `eval/build_results.py` directly from the live
scoring, so the displayed numbers cannot drift from the ranker. The quality scores show
their pre-tuning baseline (`baseline -> current`) under an explicit caption that they are
a 60-profile validation proxy, not the hidden competition score; the constraint
indicators (honeypots in top-100, runtime, validator) render as pass states.
...
`data/sample_candidates.json`, `submission.csv`, `.streamlit/config.toml`, the two
fairness caches `data/pool_demographics.json` + `data/shortlist_demographics.json`, and
`eval/results.json` (the header metrics).
...
2. Add to the Space: `app.py`, the `src/` folder, `data/sample_candidates.json`,
   `data/pool_demographics.json`, `data/shortlist_demographics.json`, `eval/results.json`,
   `submission.csv`, `.streamlit/config.toml`, and a `requirements.txt` containing the
   three lines from `requirements-sandbox.txt`. (Upload via the web UI, or `git push` to
   the Space's remote.)
...
## URL: https://redrob-ranker-team-cypher.streamlit.app/
